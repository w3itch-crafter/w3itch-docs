...
const getSidebar = (basePath=path.join(__dirname, '..')) => {
  ...
  const parseDir = (dir: string, base='') => {
    // ignore hidden files
    const files: string[] = fs.readdirSync(path.join(basePath, base)).filter((sub) => !sub.startsWith('.'));

    files.forEach((file) => {
      let sidebarChildren = sidebar;
      base.split('/').filter((e) => e).forEach((dir) => {
        sidebarChildren = sidebarChildren.find(e => e.text === dir).children;
      });
